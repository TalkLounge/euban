		...
		for name, _ in pairs(main.names) do
			if not is_ip(name) then
				...
				for _, record in ipairs(main.record) do
					...
					if index then
						table.insert(EUBan.Database[name].reasons, index, {user = record.source, status = record.expires and true or nil, time = record.time, message = record.reason})
					end
				end
				if main.banned then
  ...
  for name, main in pairs(EUBan.Database) do
    banned, account, time = EUBan.status(main)
        ...
        if ip == playerip then
				...
				for _, playerip in ipairs(EUBan.Database[playername].ips) do
					if ip == playerip then
						return "Banned by ip: ".. convert_time(time) .." Reason: ".. main.reasons[#main.reasons].message
					end
				end
...
local function record_login(playername, playerip)
  ...
  EUBan.Database[playername] = {ips = {shortip}}
end
...
function EUBan.is_whitelisted(playername)
	if EUBan.Whitelist and not minetest.get_player_privs(playername).privs and not EUBan.Database[playername].whitelist then
...
function EUBan.is_limited(playername)
	local accounts = EUBan.accounts(playername)
...
minetest.register_on_prejoinplayer(function(name, ip)
    ...
    if banned then
      return banned
		...
		local limit = EUBan.is_limited(name)
		if limit then
			return limit
		end
end)
...
function EUBan.accounts(playername)
	local names = {}
	for name, main in pairs(EUBan.Database) do
		local exception = false
		for _, name in ipairs(main.exceptions or {}) do
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
  local playername = player:get_player_name()
	...
	if fields.euban_bany_account and fields.euban_bany_account:sub(1, 3) == "DCL" and #form[playername].euban_bany_account ~= 0 then
		if not form[playername].euban_bany_disaccount then
...
local function form_limit(playername, fields)
	...
	return "size[2.4,2.9]" ..
				 ...
				 "button[1.4,0.3;1.1,1;euban_limit_search;Search]" ..
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
  local playername = player:get_player_name()
	...
	if fields.euban_limit_save then
		if tonumber(fields.euban_limit_count) ~= nil and tonumber(fields.euban_limit_count) >= 0 then
